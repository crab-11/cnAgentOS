...
cnAgentOS 是一个基于 Python Tornado 框架开发的 Web 应用系统，采用 MVC 分层架构和 B/S 部署模式。当前阶段重点完成了后端管理侧基础能力，包括后台登录、后台管理框架、用户管理、角色管理、功能/权限管理和用户角色设置。
...
│   │   └── admin_permission.html   # 功能/权限管理页
│   └── static/
│       ├── images/
...
| `/admin/permission` | GET | 功能/权限管理页面 |
| `/admin/api/permission/list` | GET | 功能/权限列表 |
| `/admin/api/permission/tree` | GET | 功能/权限树 |
...
| 用户管理 | 已完成 | 增删改查、批量删除、分页、状态、角色设置 |
| 用户角色显示 | 已完成 | 列表显示真实角色 |
| 默认 admin 保护 | 已完成 | 不能删改资料，只能改密码 |
| 角色管理 | 已完成 | 增删改查、系统角色保护 |
| 功能/权限管理 | 已完成 | 功能维护、权限树、角色映射 |
| 后台首页 | 预留 | 后续模块完成后统一完善 |
| 智能问数 | 待规划 | 后续核心业务 |
| AI 智能瞭望 | 待规划 | 后续核心业务 |
...
- 完善注册功能或明确移除注册入口。
- 增加用户操作日志。
- 后续业务模块可继续接入数字员工、模型引擎、瞭望管理、数据仓库等能力。
